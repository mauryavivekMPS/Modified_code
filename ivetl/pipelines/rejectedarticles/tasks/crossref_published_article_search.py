...
def test_file(text,data=None):
    file_path = ""
    lock_path = "/home/vivek/new_file.txt.lock"
...
def crossref_lookup(publisher, manuscript_id, data, issn_journals, tlogger,
                    mutex, target_file, matcher_debug_csv, pub_journals,Skipped_record_file_name):

    def test_file_of_thread(text,data=None,Skipped_record_file_name=None):
        file_path = Skipped_record_file_name
        lock_path = Skipped_record_file_name + ".lock"
        Thread_lock = FileLock(lock_path)
        Thread_lock.acquire()
        try:
            with open(file_path, "a") as f:
                f.write(text+'  '+str(data)+'\n')
        finally:
            Thread_lock.release()
    ...
    if title is None or not title.strip():
        ...
        test_file_of_thread("No Title Manuscripts_id ",data['manuscript_id'],Skipped_record_file_name)
        return
    ...
    if not author_last_names:
        ...
        test_file_of_thread("NO Author Manuscript_id",data['manuscript_id'],Skipped_record_file_name)
        return
...
class XREFPublishedArticleSearchTask(Task):
    ...
    def run_task(self, publisher_id, product_id, pipeline_id, job_id, work_folder, tlogger, task_args):
        ...
        # test_file("FILE NAME",target_file_name)
        ...
        with codecs.open(file, encoding="utf-16") as tsv:
            for line in csv.reader(tsv, delimiter="\t"):
                ...
                if tid in running:
                    running[tid].join()
                # test_file("Create Thread",str([publisher, manuscript_id, data,
                ...
                running[tid] = threading.Thread(
                                target=crossref_lookup, args=(
                                    ...
                                    issn_journals, tlogger,
                                    mutex, target_file, matcher_debug_csv,
                                    pub_journals,Skipped_record_file_name)
                    )
